...
Cloudflare now serves your application in an isolated browser for users who match your policies.
...
If you do not provide `<url>`, Cloudflare shows users a Cloudflare Zero Trust landing page where they can input a target URL or search for a website.
...
- **HTTP requests**: [Gateway activity logs](/cloudflare-one/insights/logs/gateway-logs/) record traffic from the remote browser to the Internet.
- **DNS queries**: [Gateway activity logs](/cloudflare-one/insights/logs/gateway-logs/) record DNS queries from the remote browser.
