...
5.  **Update `package.json`**
    ...
    :::caution[Avoid infinite build loops]
    Do not set your `build` script to `opennextjs-cloudflare build`. The `opennextjs-cloudflare build` command internally runs `npm run build`, so this creates an infinite loop and causes your build to fail. For most projects, `next build` is the correct value.
    :::

    <Details header="Usage">
    	- `build`: Runs the Next.js build process. This is called internally by
    	`opennextjs-cloudflare build`. - `preview`: Builds your app and serves it
    	locally, allowing you to quickly preview your app running locally in the
    	Workers runtime, via a single command. - `deploy`: Builds your app, and
    	then deploys it to Cloudflare - `cf-typegen`: Generates a
    	`cloudflare-env.d.ts` file at the root of your project containing the
    	types for the env.
    </Details>
