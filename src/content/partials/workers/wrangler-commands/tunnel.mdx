...
Wrangler manages the [cloudflared](/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/) binary automatically. On first use, Wrangler will prompt you to download `cloudflared` to a local cache directory. You can skip this by installing `cloudflared` yourself and adding it to your `PATH`, or by setting the `WRANGLER_CLOUDFLARED_PATH` environment variable to point to an existing binary.

:::note

Most tunnel commands require an API token with **Cloudflare Tunnel: Edit** permission. OAuth login via `wrangler login` does not currently include tunnel permissions.
Create an API token at the [Cloudflare dashboard](https://dash.cloudflare.com/profile/api-tokens) with **Account > Cloudflare Tunnel > Edit**, then set `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` as environment variables.
The `tunnel run` and `tunnel quick-start` commands do not require API authentication when a tunnel token is provided directly.

:::

<AnchorHeading title="`tunnel create`" slug="tunnel-create" depth={3} />

Create a new remotely managed [Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/).
...
Tunnels created via Wrangler are always **remotely managed** — configure them in the [Cloudflare dashboard](https://one.dash.cloudflare.com/) or via the API.
...
Run a Cloudflare Tunnel using the [cloudflared](/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/) daemon. This starts a persistent connection between your local machine and Cloudflare's network.
...
- `--url` <Type text="string" /> <MetaInfo text="optional" />
  - The local URL to expose through the tunnel (for example, `http://localhost:3000`).
...
```sh
npx wrangler tunnel run my-app --url http://localhost:3000
```

**By token** (no API authentication needed — useful for CI/CD or remote servers):

```sh
npx wrangler tunnel run --token eyJhIjoiNGE2MjY... --url http://localhost:3000
```
...
Start a free, temporary tunnel without a Cloudflare account using [Quick Tunnels](/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/). This is useful for quick demos, testing webhooks, or sharing local development servers.
