...
When an HTTP policy applies the Isolate action, Browser Isolation transparently serves the user's web browser an HTML compatible remote browser client. Apply isolation policies to requests that include `Accept: text/html*`. This allows Browser Isolation policies to co-exist with API traffic.
